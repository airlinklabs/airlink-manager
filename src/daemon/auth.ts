...
export function fingerprint(ip: string, userAgent: string, appSecret: string): string {
  return new Bun.CryptoHasher("sha256").update(`${ip}:${userAgent}:${appSecret}`).digest("hex");
}
...
async function verifyViaSu(username: string, password: string): Promise<boolean> {
  const proc = Bun.spawn(["su", "-c", "true", username], {
    stdin: "pipe",
    stdout: "pipe",
    stderr: "pipe"
  });
...
export function sessionAuth(deps: AuthDeps) {
  return createMiddleware<AirlinkEnv>(async (c, next) => {
    ...
    const expected = fingerprint(ip, ua, deps.appSecret());
